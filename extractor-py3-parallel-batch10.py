# For running first 10 timesteps!!
...
# Adapted for monthly climate data: Nolan Townsend, Robyn Holmes
# Adapted for multiprocess: Luis Garnica (lagarnicachavira@utep.edu)
# Adapted for batch processing: Robyn Holmes
...
def process_cdf(cdf_file, inputSpace):
    print("Sub-Processing CDF: " + cdf_file)
    ...
    try:
        ...
        # paths
        #inputSpace = "H:\\Documents\\Research\\netcdf-inputs"
        outputSpace = "H:\\Documents\\Research\\netcdf-outputs"
        # only the name, no extension for raster files!
        zones = "H:\\Documents\\Research\\netcdf-processor\\shapes\\sub_basins"
        ...
        if cdf_file.startswith("Extraction_"):
            ###DAILY new STEPS
            # inputs is hardcoded here, need to check use of shares variables
            # print("input path: "  + inputSpace + "\\" + cdf_file + "\n")
            nc_FP = arcpy.NetCDFFileProperties(inputSpace + "\\" + cdf_file)
            nc_Dim = nc_FP.getDimensions()

            # extracts the variable name from the file name
            fluxvar = cdf_file[11:-3] 
            ...
            for dimension in nc_Dim:
                if dimension == "time":
                    ...
                    for i in range(0, 10): # for testing a small amount of rows
                        ...
                        # the rest.
                        # print "Creating " + fluxvar + " raster for " + justdate + " " + cdf_file + "..."####
                        rasterFluxvar = arcpy.MakeNetCDFRasterLayer_md (cdf_file, fluxvar, "longitude", "latitude", os.path.join(outputSpace, fluxvar), "", dimension_values, valueSelectionMethod, "CENTER")
                        ...
                        # print "Creating " + fluxvar + " table for " + justdate + " " + cdf_file + "...\n"####
                        iotype = 'in_memory\\'  
...
def parallel_processing(clmScn, inputSpace):
    start_time = time.time()
    print("Starting new climate scenario...")
    ...
    try:
        ...
        #fileList = []
        #fileList.append(f for f in os.listdir(inputSpace) if f.endswith('.nc'))
        ...
        pool.map(functools.partial(process_cdf, inputSpace=inputSpace), fileList)
        ...
        summaryFile = clmScn ##change name to folder name here
        ...
        arcpy.TableToExcel_conversion(yearTable, os.path.join(outputSpace, summaryFile + "_test10.xls"), "", "")
...
def main():
    ...
    try:
        ...
        # input path
        inputSpace = "H:\\Documents\\Research\\netcdf-inputs" # folder containing climate scenario folders w/ cdf files

        inputFolders = os.listdir(inputSpace)
        for clmScn in inputFolders:
            clmScnPath = os.path.join(inputSpace, clmScn)
            parallel_processing(clmScn, clmScnPath)
        
            nScns = len(inputFolders)
            index = inputFolders.index(clmScn)+1
            print("Scenario " + str(index) + " of " + str(nScns) + " completed.")
        # Closes console.
        input("Script completed. Press Enter to quit.\n")
